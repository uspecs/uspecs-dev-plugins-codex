...
- Insert the `Refs:` block from `@artdef_change_refs` between the H1 and `## Why` (?fetchable_issue)
- Under `--fetchable`, populate `## Why` and `## What` by distilling the fetched issue in the change's terms; do not restate the issue body verbatim (?fetchable_issue)
